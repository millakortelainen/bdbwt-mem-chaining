...
void RMaxQTree::init(int node, int b, int e, int *keys) {
    ...
    //std::cout << "RMAX INIT: set node: " << node << " to key: " << keys[b] << "where b= " << b << std::endl;
  }
	else {
		// split
...
void RMaxQTree::updateTree(int key, int j, int Cj, int node, int b, int e) {
  //std::cout << "RMAX: UpdateTree top, node.key = " << tree[node].key << " want: "<< key << "b,e = " << b << "," << e<< std::endl;
	// Ended in a leaf
	if (b == e) {
    //std::cout << "RMAX: update leaf with leaf-key" << tree[node].key << " and update-key " << key <<std::endl;
		if(tree[node].key == key) {
      //std::cout << "RMAX: updated key: " << tree[node].key << " with value Cj = " << Cj << " and j = " << j << std::endl;
			tree[node].j = j;
			tree[node].Cj = Cj;
    }
	}
...
std::pair<int,int> RMaxQTree::queryTree(int i, int j, int node, int b, int e) {
    ...
    //std::cout << "RMAX:" << keys[b] << " > =" << i << " && " << keys[e] << " <= " << j << std::endl;
